...
<html lang="en">
  ...
  <body style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif; margin: 40px;">
    ...
    <p>
      Model loaded: {{ model_loaded }} &mdash;
      Model path: {{ model_path }}
    </p>

    <form action="/predict-form" method="post">
      <label for="message">Enter text to classify:</label><br><br>
      <textarea id="message" name="message"
                style="width: 100%; max-width: 800px; height: 150px;">{{ request.form.message if request and request.form.get('message') else '' }}</textarea>
      <br><br>
